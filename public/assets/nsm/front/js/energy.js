...
{
    $.post(getEnergyURL,{type:type}, function (data) {
        // Create the chart
        seriesdata = [{
            type: 'area',
            name: name,
...
{
    $('table.compare-energy').hide();
    if(table===undefined){
        table = $('table.energy').DataTable({
            processing: true,
            serverSide: false,
            filter:false,
            paging: false,
...
function swapElement(array, energyA, energyB) {
    var tmp = array[energyA];
    array[energyA] = array[energyB];
    array[energyB] = tmp;
}