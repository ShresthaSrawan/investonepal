...
function renderButton(chart, button, i) {
	...
	if(chart.rangeSelector)	{
		xOffset = chart.rangeSelector.inputEnabled ? chart.rangeSelector.inputGroup.offset : 0;
		y = chart.plotTop - (chart.rangeSelector.inputEnabled ? 23 + buttonSize : 0) + userOffset[1];
	} else {
		xOffset = 0;
		y = chart.plotTop + userOffset[1];
	}
        ...
        events: {
                ...
                storeAnnotation: function(event, annotation, chart) {
					...
					if((!isOldIE && event.button === 0) || (isOldIE && event.button === 1)) {
						...
						addEvent(document, 'dblclick', function(e){
								annotation.destroy();
						});
